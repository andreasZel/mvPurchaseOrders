...
var ulist = document.getElementById("listOfP");
var RowProductSKU = document.getElementById("PurchaseOrderRowProductSKU");
var RowQuantity = document.getElementById("PurchaseOrderRowQuantity");
var PriceWithoutTaxOrDiscount = document.getElementById("PurchaseOrderRowUnitPriceWithoutTaxOrDiscount");
var TotalAmount = document.getElementById("PurchaseOrderRowTotalAmount");
var OrderAddress = document.getElementById("PurchaseOrderAddress");
var ContactPerson = document.getElementById("PurchaseOrderContactPerson");
var OrderStatus = document.getElementById("PurchaseOrderStatus");
var popup = document.getElementById("myPopup");
var new_tbl = document.getElementById("tbl");
...
function displayList() {
    ...
    for ( i = 0; i < DataList.mvPurchaseOrders.length; i++) {
        // create an li for each purchase
        // give the id of the Purchase as the element id to pass it to the ShowMoreInfo function
        var Purchase_li = document.createElement('li'); 
        Purchase_li.id = DataList.mvPurchaseOrders[i].PurchaseOrderId;
        
        //append it to the ul in html file
        Purchase_li.innerHTML = '{ ' + DataList.mvPurchaseOrders[i].PurchaseOrderTypeAbbreviation + ' }-{ ' + DataList.mvPurchaseOrders[i].PurchaseOrderNo + ' }';
        ulist.appendChild(Purchase_li);

        //add an onclick eventlistener for the popup
        Purchase_li.addEventListener('click', function (){
            showMoreInfo(this);
        });
    }
}
...
function showMoreInfo(P_id) {
    ...
    for ( i = 0; i < DataList.mvPurchaseOrders.length; i++) {
        if (id == DataList.mvPurchaseOrders[i].PurchaseOrderId) {
            
            OrderAddress.innerHTML = DataList.mvPurchaseOrders[i].PurchaseOrderAddress; 
            ContactPerson.innerHTML = DataList.mvPurchaseOrders[i].PurchaseOrderContactPerson;
            OrderStatus.innerHTML = DataList.mvPurchaseOrders[i].PurchaseOrderStatus;

            for ( j = 0; j < DataList.mvPurchaseOrders[i].PurchaseOrderDetails.length; j++) {

                var new_tr = document.createElement('tr');new_tr.innerHTML = "............................................";new_tbl.appendChild(new_tr); //empty row

                var new_tr = document.createElement('tr');
                new_tr.innerHTML = DataList.mvPurchaseOrders[i].PurchaseOrderDetails[j].PurchaseOrderRowProductSKU;
                new_tbl.appendChild(new_tr);

                var new_tr = document.createElement('tr');
                new_tr.innerHTML = DataList.mvPurchaseOrders[i].PurchaseOrderDetails[j].PurchaseOrderRowQuantity;
                new_tbl.appendChild(new_tr);

                var new_tr = document.createElement('tr');
                new_tr.innerHTML = DataList.mvPurchaseOrders[i].PurchaseOrderDetails[j].PurchaseOrderRowUnitPriceWithoutTaxOrDiscount;
                new_tbl.appendChild(new_tr);

                var new_tr = document.createElement('tr');
                new_tr.innerHTML = DataList.mvPurchaseOrders[i].PurchaseOrderDetails[j].PurchaseOrderRowTotalAmount;
                new_tbl.appendChild(new_tr);
            }
        }
    }
